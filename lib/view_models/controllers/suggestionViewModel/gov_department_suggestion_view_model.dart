...
class GovtDepartmentSuggestionViewModel extends GetxController {
  // Dependencies
  ...
  /// Submit Suggestion for Govt. Department
  Future<void> submitGovtDepartmentSuggestion() async {
    ...
    try {
      ...
      if (kDebugMode) {
        dev.log('📤 Govt Department Suggestion Data: $data');
        dev.log('📎 Files: ${files.map((f) => f.name).toList()}');
      }

      // API Call
      final res = await _api.createSuggestionForGovDepartmentApi(
        data,
        headers: headers,
        files: files,
      );
      ...
      dev.log('❌ submitGovtDepartmentSuggestion Error: $e\n$st');
